  ...
  <body>
    ...
    <?php
      ...
      if(isset($_GET['food_id'])){
        ...
        while ($update = mysqli_fetch_array($sql)) {
    ...
    <div class="container p-5">
      ...
      <div class="d-flex justifiy-content-center w-100">
        <img class="w-50 mx-auto" src="../../../asset/img/food/<?= $update['foto'];?>"  alt="">
      </div>
        <form method="POST" enctype="multipart/form-data" action="">
          <div class="mb-3">
            <label class="form-label">Food ID</label>
            <input type="text" name="food_id" class="form-control" value="<?= $update['food_id'] ?>" readonly />
          </div>
          <div class="mb-3">
            <label class="form-label">Nama Produk</label>
            <input type="text" name="nama" class="form-control" value="<?= $update['nama'] ?>" required />
          </div>
          <div class="mb-3">
            <label class="form-label">Harga</label>
            <input type="number" name="harga" class="form-control" value="<?= $update['harga'] ?>" required />
          </div>
          <!-- <div class="mb-3">
            <label class="form-label">Foto</label>
            <input type="file" name="foto" class="form-control" value="<?= $update['foto'] ?>" required />
          </div> -->
          <div class="mb-3">
            <label class="form-label">Stok</label>
            <input type="number" name="jumlah" class="form-control" value="<?= $update['jumlah'] ?>" required />
          </div>
          <div class="mb-3">
            <label class="form-label">Kategori</label>
            <select name="id_kategori"  class="form-control">
            <option value="<?= $update['id_kategori'] ?>">kategori</option>
              <?php
...
<?php 
  ...
  $sql_update = "UPDATE food_item SET nama ='$nama', jumlah ='$jumlah', harga = '$harga', id_kategori ='$id_kategori' WHERE food_id = '$food_id'";
              mysqli_query($conn, $sql_update);
              echo "<div class='alert'><script>alert('Berhasil memperbaharui data produk'); window.location = 'index.php'; </script> </div>"; 
}
?>
